module RunsHelper
  ...
  def runs_table(runs, redirect_to=nil)
    runs.map do |run|
      ...
      created_at = "#{t(".t_ago", how_long: time_ago_in_words(run.created_at))}"
      ...
      if run.is_a?(Sweep) # If a sweep, get the latest finish time of its runs
        if run.runs.any? {|r| r.finish_time.blank? }
          ...
          finish_time_text = "#{t(".t_ago", how_long: time_ago_in_words(max_time))}"
        end
      else
        if run.finish_time.blank?
          ...
          finish_time_text = "#{t(".t_ago", how_long: time_ago_in_words(run.finish_time))}"
        end
      end
      #FIXME ugly fix because state tokens are used by the player as keywords
      run_state = ".rs_#{run.state}"
      [link_to("#{image(run.is_a?(Sweep) ? 'sweep_run' : 'simple_run')}#{run.name}".html_safe,
       ...
       "#{t(run_state)} #{(run.complete? ? '' : image_tag('ajax-loader.gif', :style => "vertical-align: middle"))}".html_safe,
       created_at,
       run.created_at.to_i,
       finish_time_text,
       finish_time,
